...
pub trait Pipeline<'r>: Sized {
    ...
    ///
    /// Return `None` to discard the fragment (without affecting color/depth).
    fn fragment(&self, vs_out: Self::VertexData) -> Option<Self::Fragment>;
        ...
        D: Target<Texel = f32> + Send + Sync,
    {
        ...
        unsafe fn msaa_fragment<F: FnMut(usize, usize) -> Pipe::VertexData>(
            ...
            if texel.0 != self.primitive_count {
                ...
                texel.1 = Some(self.pipeline.fragment(get_v_data(x, y)).unwrap());
            }
        ...
        unsafe fn emit_fragment<F: FnMut(f32, f32) -> Pipe::VertexData>(
            ...
            if self.write_pixels {
                let frag = if self.msaa_level == 0 {
                    if let Some(frag) = self.pipeline.fragment(get_v_data(x as f32, y as f32)) {
                        frag
                    } else {
                        return;
                    }
